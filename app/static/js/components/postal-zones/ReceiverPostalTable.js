// ReceiverPostalTable组件
window.ReceiverPostalTable = {
  template: `
    <div class="receiver-postal-table">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div class="d-flex align-items-center gap-3">
          <h3>目的邮编管理</h3>
        </div>
        <div class="d-flex gap-2">
          <button class="btn btn-success" @click="importReceiverPostal">
            <i class="bi bi-file-earmark-excel"></i> 导入目的邮编
          </button>
          <button class="btn btn-primary" @click="openCreateModal">
            <i class="bi bi-plus"></i> 添加目的邮编
          </button>
        </div>
      </div>
      ...
      <div class="table-responsive">
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="postal in receiverPostals" :key="postal.id">
      ...
      <!-- 添加/编辑邮编的模态框 -->
      <div class="modal fade" id="receiverPostalModal" tabindex="-1">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">{{ isEdit ? '编辑目的邮编' : '添加目的邮编' }}</h5>
  ...
  methods: {
    async fetchReceiverPostals() {
      try {
        const response = await axios.get('/api/postal-zones/receiver')
        this.receiverPostals = response.data
      } catch (error) {
        console.error('获取目的邮编列表失败:', error)
        alert('获取目的邮编列表失败')
      }
    },
    async fetchZones() {
    ...
    async handleSubmit() {
      try {
        if (this.isEdit) {
          await axios.put(`/api/postal-zones/receiver/${this.editId}`, this.form)
        } else {
          await axios.post('/api/postal-zones/receiver', this.form)
        }
        this.modal.hide()
        await this.fetchReceiverPostals()
      } catch (error) {
    ...
    async deletePostal(postal) {
      if (confirm(`确定要删除这条邮编记录吗？`)) {
        try {
          await axios.delete(`/api/postal-zones/receiver/${postal.id}`)
          await this.fetchReceiverPostals()
          alert('删除成功')
        } catch (error) {
    ...
    async importReceiverPostal() {
      ...
      input.onchange = async (event) => {
        ...
        try {
          await axios.post('/api/postal-zones/receiver/import', formData, {
            headers: {
  ...
  mounted() {
    this.modal = new bootstrap.Modal(document.getElementById('receiverPostalModal'))
    this.fetchReceiverPostals()
    this.fetchZones()
  }
} 